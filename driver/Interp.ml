...
let some_expr = Eval(Vptr(Mem.nullptr Memimpl.mem_ops, Int.zero), Tvoid)
...
module StateSet =
  Set.Make(struct
             type t = (Memimpl.mem, ExtFunImpl.external_function) state * Determinism.world
...
let diagnose_stuck_expr p ge w f a kont e m =
  ...
  if found then true else begin
    let l = Cexec.step_expr Memimpl.mem_ops ge e w k a m in
...
let do_step p prog ge time (s, w) =
  ...
  match Cexec.at_final_state Memimpl.mem_ops s with
  | Some r ->
      if !trace >= 1 then
  ...
  | None ->
      let l = Cexec.do_step Memimpl.mem_ops ge w s in
...
let fixup_main p =
  ...
  | None ->
      ...
      None
  | Some main_fd ->
      match type_of_fundef ExtFunImpl.ef_ops main_fd with
...
let execute prog =
  ...
  | Some prog1 ->
      ...
      match Genv.init_mem Memimpl.mem_ops wprog with
      | None ->
          fprintf p "ERROR: World memory state undefined@."
      | Some wm ->
      match Cexec.do_initial_state Memimpl.mem_ops prog1 with
      | None ->
